#!/usr/bin/env python3
"""Multi_dataset_benchmark.ipynb'nin BASSIZ (headless) surumu — BAYRAK GEREKMEZ.

Neden var: notebook hucresi Jupyter kernel'ine baglidir; tarayici kapanir ya da kernel
koparsa is olur. Bu script nohup/tmux altinda calisir — SSH'i kesebilir, bilgisayarini
kapatabilirsin. Sonuclar diske ve W&B'ye akmaya devam eder.

TEK SEFERLIK HAZIRLIK (Lightning Studio terminalinde):

    wandb login                  # anahtari bir kez yapistir, ~/.netrc'ye yazilir

CALISTIRMA — arguman yok:

    cd ~/Colored_Point_Cloud_Completion
    nohup python scripts/run_benchmark.py > bench.log 2>&1 &
    echo $! > bench.pid

    tail -f bench.log            # izleme; Ctrl-C sadece izlemeyi keser
    kill $(cat bench.pid)        # durdurmak istersen

Koparsa AYNI komutu tekrar calistir: egitilmis modeller ve hesaplanmis model sonuclari
atlanir, kaldigi yerden devam eder.

Ayar degistirmek icin asagidaki CONFIG blogunu duzenle. (Istersen ayni isimli
--bayraklarla da gecici olarak ezebilirsin, ama gerekmiyor.)
...
CONFIG = dict(
    datasets        = ["ours", "densepoint"],   # + "omniobject3d", "3dcompat" (erisim alinca)
    models_per_cat  = 200,     # kategori basina yuklenecek model (None = tamami)
    eval_n          = None,    # degerlendirilecek test modeli (None = TAMAMI)
    ddpm_epochs     = 300,
    force_retrain   = False,
    skip_env        = False,   # ortam zaten kuruluysa True yap -> ~10 dk kazanc
)
# ═══════════════════════════════════════════════════════════════════════════════════════════
...
_a = ap.parse_args()
for k, v in vars(_a).items():                      # verilen bayrak CONFIG'i ezer
    if v not in (None, False):
        CONFIG[k] = v
ARGS = argparse.Namespace(**CONFIG)
print("CONFIG:", CONFIG, flush=True)

# --- W&B kimligi: once `wandb login` onbellegi, sonra env; ikisi de yoksa dur ---
def _wandb_ready():
...
for _root in ("/teamspace/studios/this_studio", os.getcwd()):
    if os.path.isdir(_root):
        WORK = _root; break
DATA_ROOT = os.path.join(WORK, "pcc_data")     # indirilen veri setleri
OUT_ROOT  = os.path.join(WORK, "pcc_out")      # checkpoint + sonuc + rapor
