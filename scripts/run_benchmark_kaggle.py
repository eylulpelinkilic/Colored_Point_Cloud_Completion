#!/usr/bin/env python3
"""Multi_dataset_benchmark.ipynb'nin KAGGLE surumu — gercek bassiz kosu, bayrak gerekmez.

Neden Kaggle: Colab tarayici kapaninca runtime'i olduruyor (Pro+ haric). Kaggle'in
"Save Version -> Save & Run All (Commit)" secenegi notebook'u SUNUCUDA bastan sona
kosturur; tarayiciyi ve bilgisayari kapatabilirsin. Haftada 30 saat GPU, oturum
basina 12 saat.

TEK SEFERLIK HAZIRLIK (Kaggle notebook'unda, sag panel):
  1. Settings -> Accelerator -> GPU (T4 x2 ya da P100)
  2. Settings -> Internet -> ON          <-- git clone / HF / W&B icin ZORUNLU
  3. Add-ons -> Secrets -> yeni secret:  WANDB_API_KEY = <anahtarin>

CALISTIRMA — notebook'a tek hucre:

    !git clone -q https://github.com/eylulpelinkilic/Colored_Point_Cloud_Completion.git /kaggle/temp/pcc
    !cd /kaggle/temp/pcc && git checkout -q Pelin
    !pip install -q wandb
    !cd /kaggle/temp/pcc && python scripts/run_benchmark_kaggle.py --eval-n 15

Sonra: Save Version -> "Save & Run All (Commit)" -> tarayiciyi kapat.

DEVAM ETME (oturumlar arasi): bir onceki kosunun ciktisini
"Add data -> Your Work -> Notebook Output" ile bu notebook'a INPUT olarak ekle.
Script /kaggle/input altinda results.jsonl ve *.pt arar, bulursa hesaplanmis
modelleri ve egitilmis agirliklari atlar.

DIZIN DUZENI
  /kaggle/temp/...      klon, PoinTr, HF onbellegi   (KAYDEDILMEZ, 12 saat sonra silinir)
  /kaggle/working/...   sonuclar, checkpointler, rapor (version ciktisi olarak KAYDEDILIR)
...
# --- W&B: Kaggle Secrets -> env -> netrc ---
if not os.environ.get("WANDB_API_KEY"):
    try:
        from kaggle_secrets import UserSecretsClient
        os.environ["WANDB_API_KEY"] = UserSecretsClient().get_secret("WANDB_API_KEY")
        print("W&B anahtari Kaggle Secrets'tan alindi", flush=True)
    except Exception as e:
        print(f"Kaggle Secrets okunamadi ({type(e).__name__})", flush=True)
...
if not _wandb_ready():
    sys.exit("W&B kimligi yok. Kaggle'da: Add-ons -> Secrets -> WANDB_API_KEY ekle "
             "(commit modunda interaktif login mumkun degil).")

# --- onceki kosunun ciktisi INPUT olarak eklendiyse ondan devam et ---
def _seed_from_inputs(out_root):
    os.makedirs(out_root, exist_ok=True)
    seeded = []
    for src in glob.glob("/kaggle/input/*/**/results.jsonl", recursive=True):
        dst = os.path.join(out_root, "results.jsonl")
        if not os.path.exists(dst):
            shutil.copy(src, dst); seeded.append(os.path.basename(src))
    for src in glob.glob("/kaggle/input/*/**/*.pt", recursive=True):
        dst = os.path.join(out_root, os.path.basename(src))
        if not os.path.exists(dst):
            shutil.copy(src, dst); seeded.append(os.path.basename(src))
    if seeded:
        print(f"onceki kosudan devam: {len(seeded)} dosya kopyalandi", flush=True)
